...
import com.it.util.TimeCalcUtil;
import net.sf.json.JSONObject;


public class HttpHelperAsync {
	
	private static Logger logger = Logger.getLogger(HttpHelperAsync.class);
	...
	private static class HttpHelperAsyncClient {
		...
		private Logger logger = Logger.getLogger(HttpHelperAsyncClient.class);
	...
	private static abstract class HttpAsyncRequest {
		...
		public Object setParameter(JSONObject parameters) {
			logger.info(TimeCalcUtil.getReqType()+" Params: "+ parameters);
			return parameters;
		}
		...
		public Object setParameter(Map<String, Object> parameters) {
			...
			logger.info(TimeCalcUtil.getReqType()+" Params: "+ paramsBuffer);
			return params;
		}
		...
		public Response request(String url, Headers headers, String parameters, long timeoutMillis) throws Exception {
			try {
				...
				if (contentType.contains(APPLICATION_JSON)) {
					response = applicationJSON(url, headers, JSONObject.fromObject(parameters), timeoutMillis);
				} else {
				...
				//System.out.println(response.getContent()+"4444");
				return response;
		...
		private UrlParam parseUrlParam(String url, String parameters) {
			...
			if(null != parameters && parameters!="") {            	
                ...
                for(int i = 0;i < str.length;i++) {
                	...
                	//System.out.println(params.get(str1[0]));
                }           	
            }							
			if (hasUrlParam) {
				String srcUrl = url;
	...
	private static class PostJSON extends HttpAsyncRequest {
		...
		public HttpEntity setHttpEntity(Object params) throws Exception {
			JSONObject jsonParams = JSONObject.fromObject(params.toString());
	...
	public static Response get(String url, Headers headers, String parameters, long timeoutMillis) throws Exception {
		TimeCalcUtil.setReqType("Get");
		return get.request(url, headers, parameters, timeoutMillis);
	}
	
	public static Response post(String url, Headers headers, String parameters, long timeoutMillis) throws Exception {
